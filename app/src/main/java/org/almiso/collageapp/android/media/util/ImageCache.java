 ...
 */
/*
    This class was taken from Saples
 */
    ...
    private void init(ImageCacheParams cacheParams) {
        ...
        if (mCacheParams.memoryCacheEnabled) {
            if (BuildConfig.DEBUG) {
                Logger.d(TAG, "Memory cache created (size = " + mCacheParams.memCacheSize + ")");
            }
                ...
                if (mCacheParams.diskCacheEnabled && diskCacheDir != null) {
                    ...
                    if (getUsableSpace(diskCacheDir) > mCacheParams.diskCacheSize) {
                        try {
                            ...
                            if (BuildConfig.DEBUG) {
                                Logger.d(TAG, "Disk cache initialized");
                            }
                        } catch (final IOException e) {
                            mCacheParams.diskCacheDir = null;
                            Logger.e(TAG, "initDiskCache - ", e);
                        }
                    }
                }
            }
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    Logger.e(TAG, "getBitmapFromDiskCache - ", e);
                } finally {
                    try {
                        if (inputStream != null) {
                            inputStream.close();
        ...
        synchronized (mDiskCacheLock) {
            if (mDiskLruCache != null) {
                try {
                    mDiskLruCache.flush();
                    if (BuildConfig.DEBUG) {
                        Logger.d(TAG, "Disk cache flushed");
                    }
                } catch (IOException e) {
            ...
            if (mDiskLruCache != null) {
                try {
                    if (!mDiskLruCache.isClosed()) {
                        ...
                        mDiskLruCache = null;
                        if (BuildConfig.DEBUG) {
                            Logger.d(TAG, "Disk cache closed");
                        }
                    }
                } catch (IOException e) {
